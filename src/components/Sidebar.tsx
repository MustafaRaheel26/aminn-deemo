...
export default function Sidebar({ currentPath }: SidebarProps) {
  ...
  const superNavItems = [
    { name: 'Super Admin Dashboard', icon: LayoutDashboard, path: '#/super-dashboard' },
  ...
  const tenantNavItems = [
    { name: 'Restaurant Overview', icon: LayoutDashboard, path: '#/restaurant-dashboard' },
  ...
  const currentNav = currentUser?.role === 'super_admin' ? superNavItems : tenantNavItems;

  return (
    <aside className="w-64 bg-white border-r border-slate-200 flex flex-col justify-between shrink-0 h-screen sticky top-0 text-slate-600 z-30" id="main-sidebar">
      {/* Top Brand Logo */}
      <div>
        ...
        <div className="p-4 mx-3 mt-4 rounded-xl bg-slate-50 border border-slate-200 text-xs">
          <div className="flex items-center gap-2 mb-1.5 text-slate-400 font-medium">
            ...
            <span className="uppercase text-[9px] font-mono tracking-wide">ACTIVE TENANT SCOPE</span>
          </div>
          {currentUser?.role === 'super_admin' ? (
            activeRestaurantId ? (
              <div className="flex items-center justify-between">
                <span className="font-bold text-slate-850 truncate text-xs">{activeRest?.name}</span>
                <button 
              ...
              <span className="text-amber-600 font-semibold text-[11px]">Global Management (All tenants)</span>
            )
          ) : (
            <span className="font-bold text-slate-850 text-xs">{activeRest?.name || 'Authorized Tenant'}</span>
          )}
        </div>
        ...
        <nav className="mt-6 px-4 space-y-1">
          {currentNav.map((item, idx) => {
            const isActive = currentPath === item.path || (item.path === '#/restaurant-detail' && currentPath.startsWith('#/restaurant-detail'));
              ...
              <a
                ...
                className={`flex items-center justify-between px-3 py-2 rounded-lg text-xs font-semibold tracking-wide transition-all ${
                  isActive
                    ...
                    : 'hover:bg-slate-50 hover:text-slate-905 text-slate-600'
                }`}
                id={`sidebar-link-${idx}`}
              >
                <div className="flex items-center gap-3">
                  <IconComponent className={`h-4 w-4 ${isActive ? 'text-[#EA580C]' : 'text-slate-400'}`} />
                  <span>{item.name}</span>
                </div>
                {isActive && <ChevronRight className="h-3 w-3 text-[#EA580C]" />}
              </a>
            );
          })}
        </nav>
        ...
        <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-200 text-xs flex flex-col gap-2.5">
          <div className="flex items-center gap-3">
            ...
            <div className="overflow-hidden flex-1">
              <p className="text-slate-800 text-sm font-semibold truncate leading-tight">{currentUser?.name}</p>
