...
export default function DigitalMenuBoardPreview({ screenId }: PreviewProps) {
  ...
  const [activeLayout, setActiveLayout] = useState<'grid-two-col' | 'featured-large' | 'drinks-list'>('grid-two-col');
  ...
  const matchedCategories = categories.filter(c => c.restaurantId === selectedRestId && c.isActive);
  const matchedProducts = products.filter(p => p.restaurantId === selectedRestId && p.status === 'available');
  const matchedPromos = promotions.filter(p => p.restaurantId === selectedRestId && p.status === 'active');
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-white flex flex-col justify-between font-sans overflow-hidden select-none relative" id="menu-board-tv-preview">
      ...
      <header className="border-b border-neutral-900 bg-black/40 backdrop-blur-md h-20 px-8 flex items-center justify-between relative z-10">
        <div className="flex items-center gap-4 animate-flicker">
          <img 
            ...
            src={activeRest?.logoUrl || 'https://images.unsplash.com/photo-1498654896293-37aacf113fd9?w=150&auto=format&fit=crop'} 
            alt={activeRest?.name} 
      ...
      <main className={`flex-1 p-8 grid gap-8 overflow-hidden z-10 ${
        ...
        {activeLayout === 'grid-two-col' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 h-full auto-rows-max" id="tv-layout-grid-dual">
            {matchedCategories.slice(0, 3).map((cat) => {
              const catProds = matchedProducts.filter(p => p.categoryId === cat.id);
              return (
                <div key={cat.id} className="bg-neutral-900/50 border border-neutral-900 p-5 rounded-2xl flex flex-col justify-between">
                  <div>
                    {/* Category Header */}
                    <div className="flex items-center gap-2 pb-3.5 border-b border-neutral-800 mb-4">
                      ...
                      <h4 className="font-display font-black text-white text-base tracking-wider uppercase">{cat.name}</h4>
                    </div>
                        ...
                        <div key={p.id} className="flex gap-3 items-start group">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex justify-between items-start gap-2">
                              <span className="font-bold text-white text-xs block leading-tight truncate">{p.name}</span>
                              <span className="font-mono text-xs font-black text-orange-400 shrink-0 pl-1">
                                ${p.price.toFixed(2)}
                              </span>
                            </div>
                            <p className="text-[10px] text-neutral-400 leading-normal font-light truncate mt-0.5">{p.description}</p>
                            {p.calories && <span className="text-[9px] font-mono text-neutral-500 uppercase tracking-wider block mt-0.5">{p.calories}</span>}
                          </div>
                        </div>
                      ))}
                    </div>
        ...
        {activeLayout === 'featured-large' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full items-stretch" id="tv-layout-featured-large">
            ...
            <div className="lg:col-span-2 rounded-3xl border border-neutral-900 overflow-hidden bg-neutral-900/40 relative flex flex-col justify-end p-8 group">
              <div className="absolute inset-0">
                <img 
                  referrerPolicy="no-referrer"
                  src={matchedProducts[0]?.imageUrl || 'https://images.unsplash.com/photo-1555939594-58d7cb561ad1?w=900&auto=format&fit=crop'} 
              ...
              <div className="relative z-10 space-y-2">
                ...
                <p className="text-neutral-300 text-xs font-light max-w-xl leading-relaxed">
                  {matchedProducts[0]?.description || 'Premium grass-fed lamb minced with red peppers on steel blades.'}
                </p>
                <div className="pt-4 flex items-center gap-4">
                  <span className="text-3xl font-mono font-black text-orange-400">
                    ${matchedProducts[0]?.price.toFixed(2) || '19.50'}
                  </span>
                  <span className="text-neutral-500 font-mono text-xs uppercase bg-neutral-900/80 px-2 rounded">{matchedProducts[0]?.calories || '650 kcal'}</span>
                </div>
              </div>
            </div>
                  ...
                  {matchedProducts.slice(1, 4).map((p) => (
                    <div key={p.id} className="flex gap-3 items-center">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <span className="text-[10.5px] font-mono font-bold text-orange-400 block mt-1">${p.price.toFixed(2)}</span>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {activeLayout === 'drinks-list' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 h-full" id="tv-layout-drinks">
            <div className="bg-neutral-900/40 border border-neutral-900 rounded-3xl p-6 relative overflow-hidden flex flex-col justify-between">
              <div>
                ...
                <div className="space-y-4 text-xs font-sans">
                  {matchedProducts.filter(p => p.categoryId.toLowerCase().includes('drink') || p.categoryId.toLowerCase().includes('tea') || p.name.toLowerCase().includes('tea') || p.name.toLowerCase().includes('ayran') || p.name.toLowerCase().includes('ring') || p.categoryId.toLowerCase().includes('side')).slice(0, 5).map((p) => (
                    <div key={p.id} className="flex justify-between items-center bg-black/25 p-2 rounded-lg">
                      <div>
                        <span className="font-bold text-white text-xs">{p.name}</span>
                        <p className="text-[10px] text-neutral-450 truncate max-w-xs">{p.description}</p>
                      </div>
                      <span className="font-mono font-bold text-orange-400">${p.price.toFixed(2)}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="bg-gradient-to-br from-orange-950 via-neutral-950 to-neutral-950 border border-neutral-900 rounded-3xl p-8 flex flex-col justify-between items-center text-center">
              <div className="space-y-4">
                ...
                <h3 className="text-2xl font-display font-black text-white leading-tight uppercase">Buy 1 Get 1 Free on Ayran drinks!</h3>
                <p className="text-slate-400 text-xs font-light max-w-xs leading-relaxed">Simmered in double pots, salted sea-salt foam Ayran is perfect complement with charcoal lamb skewer.</p>
              </div>

              <div className="bg-neutral-900 border border-neutral-800 p-4 rounded-2xl w-full">
                <span className="text-[10px] uppercase font-mono text-neutral-400">Available at all checkout branches</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="whitespace-nowrap flex animate-[marquee_20s_linear_infinite]">
          {matchedPromos.map(promo => (
            <span key={promo.id} className="inline-block px-12 font-semibold">
              🔥 {promo.name}: {promo.description} • {promo.discountText} • VALID TILL {promo.endDate} ★
            </span>
          ))}
          {/* Fallback scroll if no promotional items exist */}
          {matchedPromos.length === 0 && (
            <span className="inline-block px-12 font-semibold">
              ★ AMIN CLOUD NETWORK SYSTEM SECURE • INSTANT SCREEN SYNC FUNCTIONAL • RE-SYNC HEARTBEAT STABLE • TERMINAL BOOT CODE G-451 ★
            </span>
          )}
        </div>
      </footer>
          ...
          >
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-1">
                  ...
                  <button 
                    ...
                    className={`p-1.5 rounded font-bold text-[10px] ${selectedRestId === 'rest-mac' ? 'bg-[#EA580C] text-white' : 'bg-[#EA580C]/20 hover:bg-[#EA580C]/40 text-slate-405'}`}
                  >
                    Mac Chicken
                  </button>
                </div>
              ...
              <div className="flex justify-between items-center pt-1.5">
                <button
                  onClick={() => {
                    ...
                    if (savedUser) {
                      ...
                      window.location.hash = userObj.role === 'super_admin' ? '#/super-dashboard' : '#/restaurant-dashboard';
                    } else {
                ...
                <span className="text-[10px] text-slate-500 font-mono font-bold">1080p Streamed Link</span>
              </div>
