...
export default function DigitalScreensPage() {
  const { screens, branches, restaurants, activeRestaurantId, addScreen } =
    useData();
  ...
  const tenantBranches = branches.filter(
    (b) => b.restaurantId === (activeRestaurantId || "rest-miracle"),
  );
    ...
    <div className="space-y-6" id="digital-screens-page">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-display font-extrabold text-slate-950 tracking-tight">
            {activeRestaurantId
              ? `${currentRestObj?.name} Digital Display Nodes`
              : "SaaS TV Grid Terminal"}
          </h2>
          <p className="text-xs text-slate-500 font-light mt-0.5">
            Monitor browser endpoints, template structures, and cloud heartbeats
            of digital storefront hardware.
          </p>
        </div>
      ...
      {showAddForm && (
        <form
          ...
          className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm space-y-4 max-w-2xl"
          id="add-screen-form"
        >
          <h3 className="text-sm font-bold text-slate-800 uppercase tracking-widest font-mono">
            Bootscreen Configuration Parameters
          </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-slate-50 border border-slate-300 rounded-lg p-2.5 text-xs text-slate-700 font-semibold focus:outline-none focus:border-[#EA580C] focus:ring-1 focus:ring-[#EA580C]/35"
              >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <label className="border border-slate-200 p-3 rounded-xl flex items-center gap-3.5 cursor-pointer hover:bg-slate-50">
                  ...
                  <div>
                    ...
                    <span className="text-[10px] text-slate-400">
                      Designed for side beverage screens
                    </span>
                  </div>
                </label>
              </div>
            ...
            >
              <div className="space-y-3.5">
                ...
                <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                  <div className="flex items-center gap-2">
                    <div className="bg-slate-100 p-2 rounded-xl text-slate-500 border border-slate-200 shrink-0">
                  ...
                  <span
                    className={`px-2.5 py-1 rounded font-mono text-[10px] font-bold border ${
                      scr.status === "online"
                ...
                <div className="grid grid-cols-2 gap-4 text-xs">
                  ...
                  <div className="col-span-2">
                    <span className="block text-[10px] text-slate-400 uppercase font-mono tracking-wider">
                      Installed Display URL (Downstream Iframe)
                    </span>
                    <div className="bg-slate-50 border border-slate-150 p-2 rounded-lg font-mono text-[11px] text-slate-500 mt-1 flex flex-col sm:flex-row items-start sm:items-center justify-between overflow-hidden">
                      <span className="break-words whitespace-normal text-left w-full text-[11px] select-all">
                        {scr.url}
                      </span>
                      <span className="text-[9px] font-bold text-slate-300 font-sans uppercase pt-2 sm:pt-0 sm:pl-2 shrink-0">
                        Web Code
                      </span>
                    </div>
                  </div>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                <div className="text-[10px] font-mono text-slate-400 uppercase flex items-center gap-1">
                  ...
                  <span>
                    Template ID:{" "}
                    <b className="font-sans text-slate-600 font-bold">
                      {scr.templateId}
                    </b>
                  </span>
                </div>
      ...
      <div className="bg-slate-900 text-white p-6 rounded-3xl border border-slate-800 space-y-2 max-w-4xl shadow-xl">
        <h5 className="text-sm font-semibold tracking-wide uppercase font-mono text-orange-400">
          Commercial TV Hardware Sizing Guidelines
        </h5>
        <p className="text-xs text-slate-300 leading-relaxed font-light">
          Any consumer or industrial TV display panel (43", 55", or 65" Android
          TV, LG webOS, or Samsung Tizen) can consume this high-fidelity layout.
          Boot the TV browser, navigate to your screen's Unique Web URL, and
          trigger F11 for distraction-free edge-to-edge counter displays. Menus
          sync in real time of 250ms when kitchen pricing changes.
        </p>
      </div>
    </div>
  );
