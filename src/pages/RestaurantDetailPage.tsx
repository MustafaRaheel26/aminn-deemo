...
export default function RestaurantDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "products" | "categories" | "branches" | "screens" | "promotions"
  >("products");
  ...
  const tenantCategories = categories.filter(
    (c) => c.restaurantId === currentRestaurantId,
  );
  const tenantProducts = products.filter(
    (p) => p.restaurantId === currentRestaurantId,
  );
  const tenantBranches = branches.filter(
    (b) => b.restaurantId === currentRestaurantId,
  );
  const tenantScreens = screens.filter(
    (s) => s.restaurantId === currentRestaurantId,
  );
  const tenantPromotions = promotions.filter(
    (p) => p.restaurantId === currentRestaurantId,
  );
  ...
  return (
    <div className="space-y-6" id="restaurant-detail-page">
      ...
      {/* Detail Tab Switches */}
      <div
        className="bg-slate-200/60 p-1.5 rounded-xl grid grid-cols-2 md:grid-cols-5 gap-1 shadow-inner border border-slate-200"
        id="detail-tabs-switcher"
      >
        <button
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-4 md:p-6 min-h-[400px]">
        ...
        {activeTab === "products" && (
          <div className="space-y-4" id="pane-products">
            ...
            <div className="space-y-4">
                ...
                {processedProducts.length === 0 ? (
                  <div className="bg-white border border-slate-200 rounded-2xl p-6 text-center text-slate-500 shadow-sm">
                    ...
                    <p className="text-[11px] text-slate-400 mt-2">
                      Use the orange PDF wizard above or click "Add Product" to
                      inject catalog elements.
                    </p>
                  </div>
                ) : (
                  processedProducts.map((p) => {
                    const catSymbol = tenantCategories.find(
                      (c) => c.id === p.categoryId,
                    );
                    return (
                      <div
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <img
                            ...
                            src={p.imageUrl}
                            alt={p.name}
                            className="w-16 h-16 rounded-2xl object-cover border border-slate-150 shadow-inner"
                          />
                          <div className="min-w-0">
                            ...
                            <div className="mt-2 flex flex-wrap items-center gap-2 text-[10px]">
                              <span className="bg-slate-100 text-slate-600 font-mono px-2 py-1 rounded-full border border-slate-150">
                                {catSymbol
                                  ? catSymbol.name
                                  : "Unallocated Category"}
                              </span>
                              <span className="bg-emerald-50 text-emerald-800 font-mono px-2 py-1 rounded-full border border-emerald-100 uppercase">
                                {p.status}
                              </span>
                            </div>
                          </div>
                        ...
                        <div className="flex items-center justify-between gap-3 text-xs">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => {
                                alert(
                                  `"Edit Product" action modal would be displayed in production. Current product UUID is: ${p.id}. Feel free to use the quick Delete action or Add more items.`,
                                );
                              }}
                            ...
                            <button
                              ...
                              className="text-rose-400 hover:text-rose-600 hover:bg-rose-50 p-2 rounded-lg transition-all"
                              title="Delete Item"
                            >
              ...
              <div
                className="hidden sm:block border border-slate-200 rounded-xl overflow-hidden shadow-sm"
                id="tenant-products-table"
              >
                        ...
                        <tr>
                          ...
                          >
                            ...
                            <p className="text-[11px] text-slate-400 font-sans mt-0.5">
                              Use the orange PDF wizard above or click "Add
                              Product" to inject catalog elements.
                            </p>
                          </td>
                        </tr>
                      ) : (
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="px-6 py-3.5">
                                ...
                                <span
                                  className="block text-slate-400 font-light mt-0.5 break-words max-w-sm"
                                  title={p.description}
                                >
                                  {p.description}
                                </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {tenantCategories.map((cat) => {
                  ...
                  return (
                    <div
                      key={cat.id}
                      className="border border-slate-150 p-4 rounded-xl hover:border-[#EA580C]/40 transition-all bg-slate-50/50 flex flex-col justify-between"
                    >
                      <div>
            ...
            <div className="bg-slate-50 border border-slate-200 p-5 rounded-2xl space-y-4">
              <div>
                ...
                <p className="text-[11px] text-slate-400 mt-0.5">
                  Define screen folders to group items logically.
                </p>
              </div>
        ...
        {activeTab === "branches" && (
          <div className="space-y-4" id="pane-branches">
            <div>
              <h4 className="text-sm font-bold text-slate-800 uppercase tracking-wider font-display">
                Branch Terminal Registry
              </h4>
              <p className="text-xs text-slate-400 mt-0.5">
                Physical retail properties serving this tenant's menu displays.
              </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {tenantBranches.map((branch) => {
                const bScreens = screens.filter(
                  (s) => s.branchId === branch.id,
                );
                return (
                  <div
                    key={branch.id}
                    className="border border-slate-200 bg-white hover:border-slate-300 shadow-sm rounded-xl p-5 space-y-3.5 flex flex-col justify-between"
                  >
                    <div>
                      <div className="flex items-center justify-between">
                    ...
                    <div className="border-t border-slate-100 pt-3 flex flex-wrap gap-1.5">
                      <span className="text-[10px] font-mono text-slate-400 block w-full mb-1">
                        CONNECTED DEVICES:
                      </span>
                      {bScreens.map((scr) => (
                        <span
                          key={scr.id}
                          className="bg-slate-100 text-slate-600 font-mono text-[9px] px-2 py-0.5 rounded border border-slate-150"
                        >
                          {scr.name.split(" (")[0]}
                        </span>
                      ))}
        ...
        {activeTab === "screens" && (
          <div className="space-y-4" id="pane-screens">
            <div className="flex justify-between items-center pb-2.5 border-b border-slate-100">
              <div>
                <h4 className="text-sm font-bold text-slate-800 uppercase tracking-wider font-display">
                  Connected Digital Terminals
                </h4>
                <p className="text-xs text-slate-400 mt-0.5">
                  Allocated menu display screens running active render boards.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {tenantScreens.map((scr) => {
                const bSymbol = tenantBranches.find(
                  (b) => b.id === scr.branchId,
                );
                return (
                  <div
                    key={scr.id}
                    className="border border-slate-150 p-4 rounded-xl bg-slate-50/50 flex flex-col justify-between gap-4"
                  >
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400 font-mono mt-0.5">
                          Location: {bSymbol?.name || "Central"}
                        </p>
                      </div>

                      <span
                        className={`px-2 py-0.5 rounded font-mono text-[9px] font-bold inline-block border ${
                          scr.status === "online"
                    ...
                    <div className="bg-white border border-slate-200 p-2 rounded-lg text-xs font-mono flex items-center justify-between text-slate-500 overflow-hidden text-ellipsis">
                      <span className="truncate max-w-[200px]" title={scr.url}>
                        {scr.url}
                      </span>
                      <span className="text-[9px] uppercase font-bold text-slate-400 pl-2 shrink-0">
                        Screen Code
                      </span>
                    </div>
                    ...
                    <div className="border-t border-slate-150 pt-3 flex items-center justify-between text-[11px]">
                      <span className="text-slate-400">
                        Model Refresh Rate:{" "}
                        <b className="font-mono text-slate-600">60Hz</b>
                      </span>
        ...
        {activeTab === "promotions" && (
          <div className="space-y-4" id="pane-promotions">
            <h4 className="text-sm font-bold text-slate-800 uppercase tracking-wider font-display">
              Active campaigns & Event Slates
            </h4>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {tenantPromotions.map((promo) => {
                const pBranchList = tenantBranches.filter((b) =>
                  promo.branchIds.includes(b.id),
                );
                const pScreenList = tenantScreens.filter((s) =>
                  promo.screenIds.includes(s.id),
                );

                return (
                  <div
                    key={promo.id}
                    className="border border-slate-200 bg-white rounded-xl shadow-sm overflow-hidden flex flex-col md:flex-row shadow-sm hover:shadow-md transition-all"
                  >
                    <img
                      referrerPolicy="no-referrer"
                      src={promo.imageUrl}
                    ...
                    <div className="p-4 flex-1 flex flex-col justify-between space-y-3">
                      <div>
                        ...
                        <h5 className="font-extrabold text-slate-900 text-sm mt-1.5">
                          {promo.name}
                        </h5>
                        <p className="text-slate-400 text-[11px] leading-relaxed font-light mt-1">
                          {promo.description}
                        </p>
                      </div>
                      ...
                      <div className="pt-2 border-t border-slate-100 text-[10px] font-mono text-slate-400 space-y-1">
                        ...
                        <p>
                          Target Screens:{" "}
                          <span className="text-[#EA580C] font-bold font-sans">
                            {pScreenList
                              .map((s) => s.name.split(" (")[0])
                              .join(", ") || "All Connected Screens"}
                          </span>
                        </p>
                      </div>
                    </div>
