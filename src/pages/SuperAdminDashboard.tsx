...
export default function SuperAdminDashboard() {
  const { restaurants, branches, screens, promotions, setActiveRestaurantId } =
    useData();
  ...
  return (
    <div className="space-y-6" id="super-admin-dashboard">
      {/* Intro Banner Grid */}
      <div
        className="bg-white rounded-2xl border border-slate-200 p-6 text-slate-900 relative overflow-hidden shadow-sm"
        id="intro-banner"
      >
        ...
        <div className="max-w-2xl">
          ...
          <p className="text-xs text-slate-500 mt-2 pb-4 leading-relaxed">
            Monitor, override, and provision multi-tenant nodes. You possess
            top-level clearance to manipulate active menus, configure digital
            counter displays, parse vintage menus using OCR models, and assign
            campaigns across all connected branches instantly.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-4 text-xs font-mono border-t border-slate-100 pt-4">
          ...
          <span className="text-slate-500">
            Database Core: Relational Scale-to-Zero Node
          </span>
          <span className="text-slate-300">•</span>
          <span className="text-[#EA580C] font-bold">
            Merchant Token Session Active
          </span>
        </div>
      </div>
      ...
      >
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            ...
            <p className="text-3xl font-display font-black text-slate-950 mt-1">
              {totalRestaurants}
            </p>
            <span className="text-[10px] text-slate-400 font-mono mt-0.5 block">
              Miracle, Chef's, Mac In Chicken
            </span>
          </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            ...
            <span className="text-[10px] text-emerald-600 font-mono mt-0.5 block font-semibold">
              100% Locations Live
            </span>
          </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            ...
            <span className="text-[10px] text-emerald-600 font-mono mt-0.5 block font-semibold">
              {screens.filter((s) => s.status === "online").length} Board
              Devices Active
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 md:p-6 lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-100">
            <div>
              <h3 className="text-base font-bold font-display text-slate-950">
                Tenant Client Onboarding Registry
              </h3>
              <p className="text-xs text-slate-400">
                Manage individual subscription profiles, products catalog, and
                digital units.
              </p>
            </div>
          ...
          <div
            ...
            id="super-recent-restaurants"
          >
            {restaurants.map((rest) => {
              const bCount = branches.filter(
                (b) => b.restaurantId === rest.id,
              ).length;
              const sCount = screens.filter(
                (s) => s.restaurantId === rest.id,
              ).length;
              const pCount = promotions.filter(
                (p) => p.restaurantId === rest.id,
              ).length;
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-0.5 font-light max-w-sm truncate">
                        {rest.description}
                      </p>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 md:p-6 space-y-4">
          <div>
            <h3 className="text-base font-bold font-display text-slate-950">
              Active Display Stream
            </h3>
            <p className="text-xs text-slate-400">
              Live operational status of digital boards across terminals.
            </p>
          </div>
          ...
          <div className="space-y-3" id="super-displays-feed">
            {screens.slice(0, 4).map((screen) => {
              ...
              return (
                <div
                  key={screen.id}
                  className="bg-slate-50 border border-slate-150 p-3 rounded-xl space-y-2 text-xs"
                >
          ...
          <div className="bg-amber-50 rounded-xl p-3 border border-amber-200 text-[11px] text-amber-800 space-y-1">
            ...
            <p className="text-amber-700">
              Any product added here updates the visual layout and database
              schema immediately. Digital counters consume this downstream.
            </p>
          </div>
        </div>
      </div>
